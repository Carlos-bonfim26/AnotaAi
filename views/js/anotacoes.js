                    ...
                    secaoAnotacoes.innerHTML += `
                        <article id="card-anotacao-${a.id_anotacao}" class="card_anotacao">
                            ...
                            <form class="anotacao_edicao" style="display:none" id="anotacao_edicao-${a.id_anotacao}">
                                <input type="text" value="${a.descricao_anotacao}" spellcheck="true">
                            </form>
                        </article>
            `;
                });
...
async function excluirAnotacao(id) {
    ...
    await api.delete(`/deletarAnotacao/${id}`)
        .then(response => {
...
async function salvarAnotacao(obj, id) {
    ...
    await api.patch(`/atualizarAnotacao/${id}`,
        JSON.stringify({ descricao_anotacao: anotacaoE.children[0].value }),
        {
        ...
        .then(response => {
            ...
            if (response.status === 200) {
                anotacaoE.children[0].value = response.data.anotacaoBuscada.descricao_anotacao;
                anotacaoV.children[1].innerText = response.data.anotacaoBuscada.descricao_anotacao;
...
async function cadastrarNovaAnotacao(event) {
    ...
    api.post("/criarAnotacao",
        JSON.stringify({
            descricao_anotacao: event.srcElement[0].value,
                ...
                secaoAnotacoes.innerHTML += `
                       <article id="card-anotacao-${response.data.novaAnotacao.id_anotacao}" class="card_anotacao">
                            ...
                            <form class="anotacao_visualizacao" id="anotacao_visualizacao-${response.data.novaAnotacao.id_anotacao}">
                                ...
                                >
                                ${response.data.novaAnotacao.descricao_anotacao}
                                </label>

                            </form>

                            <form class="anotacao_edicao" style="display:none" id="anotacao_edicao-${response.data.novaAnotacao.id_anotacao}">
                                <input type="text" value="${response.data.novaAnotacao.descricao_anotacao}" spellcheck="true">
                            </form>
                        </article>
                `;
            }
